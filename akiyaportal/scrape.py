"""Scrape Akiya Portal (akiyaportal.com) saved searches.

    python3 -m akiyaportal.scrape                          # enabled akiyaportal searches
    python3 -m akiyaportal.scrape jp-houses-10k
    python3 -m akiyaportal.scrape jp-houses-10k --no-details --max-pages 1
    python3 -m akiyaportal.scrape jp-houses-10k --detail-limit 20
...
from . import adapter as _adapter  # noqa: F401  — register akiyaportal
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape Akiya Portal (akiyaportal.com) into SQLite.")
    ap.add_argument("search", nargs="*", help="search names from searches.json (default: enabled akiyaportal)")
    ...
    ap.add_argument("--detail-limit", type=int, default=None,
                    help="fetch at most N detail pages this run (resumable; list order)")
    ...
    if foreign:
        ap.error(f"not an akiyaportal search: {', '.join(foreign)}. "
