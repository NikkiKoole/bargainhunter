"""Local web UI.
    ...
    python3 -m core.serve               # same

Serves the static page from franimo/web/ plus a small JSON API. The whole
result set is sent to the browser in one go and all filtering/sorting happens
client-side.
"""
        ...
        for c in JSON_COLS:
            ...
            if d.get(c):
                try:
                    ...
                    d[c] = None
        # A drop the portal itself advertises (strikethrough price) counts too.
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Serve the local bargainhunter browser UI.")
    ...
    print(f"bargainhunter UI on {url}  (db: {args.db})\nCtrl-C to stop")
    if not args.no_open:
        webbrowser.open(url)
    try:
        srv.serve_forever()
