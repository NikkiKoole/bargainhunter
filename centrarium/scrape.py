"""Scrape Centrarium (centrarium.com) saved searches.

    python3 -m centrarium.scrape                          # enabled centrarium searches
    python3 -m centrarium.scrape ct-me-houses-100k
    python3 -m centrarium.scrape ct-me-houses-100k --no-details --max-pages 1
...
from . import adapter as _adapter  # noqa: F401  — register centrarium
from .http import BASE, DEFAULT_GAP, Fetcher
from .parse import SOURCE, parse_detail, parse_list
...
def crawl_search(con, fetcher: Fetcher, name: str, spec: dict, max_pages: int | None,
    ...
    while url:
        ...
        if max_pages and pages >= max_pages:
            break
        # lowprice is cheap-first: once a full page is over skip.above,
        # later pages only get more expensive. Still follow next_url when
        # the page was empty of cards (shouldn't happen) so we don't
        # truncate a hole.
        if (skip.get("above") is not None and page["listings"]
                and page_kept == 0):
            break
        url = page["next_url"]
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape Centrarium (centrarium.com) into SQLite.")
    ap.add_argument("search", nargs="*",
                    help="search names from searches.json (default: enabled centrarium)")
    ...
    ap.add_argument("--workers", type=int, default=1,
                    help="detail-page workers (default 1 — robots crawl-delay is 5s)")
    ap.add_argument("--gap", type=float, default=DEFAULT_GAP,
                    help="minimum seconds between requests (robots.txt Crawl-delay: 5)")
    ...
    if foreign:
        ap.error(f"not a centrarium search: {', '.join(foreign)}. "
