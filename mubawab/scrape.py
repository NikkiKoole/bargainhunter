"""Scrape Mubawab (mubawab.ma) saved searches.

    python3 -m mubawab.scrape                          # enabled mubawab searches
    python3 -m mubawab.scrape mw-ma-houses-100k
    python3 -m mubawab.scrape mw-ma-houses-100k --no-details --max-pages 1
...
from . import adapter as _adapter  # noqa: F401  — register mubawab
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape Mubawab (mubawab.ma) into SQLite.")
    ap.add_argument("search", nargs="*",
                    help="search names from searches.json (default: enabled mubawab)")
    ...
    if foreign:
        ap.error(f"not a mubawab search: {', '.join(foreign)}. "
