"""Scrape home.ge saved searches.

    python3 -m homege.scrape                          # enabled homege searches
    python3 -m homege.scrape hg-ge-houses-100k
    python3 -m homege.scrape hg-ge-houses-100k --no-details --max-pages 1
    python3 -m homege.scrape hg-ge-houses-100k --detail-limit 20
...
from . import adapter as _adapter  # noqa: F401  — register homege
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape home.ge (Georgia) into SQLite.")
    ap.add_argument("search", nargs="*",
                    help="search names from searches.json (default: enabled homege)")
    ...
    if foreign:
        ap.error(f"not a homege search: {', '.join(foreign)}. "
