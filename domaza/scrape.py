"""Scrape Domaza (domaza.com) saved searches.

    python3 -m domaza.scrape                          # enabled domaza searches
    python3 -m domaza.scrape dz-me-houses
    python3 -m domaza.scrape dz-al-houses --no-details --max-pages 1
    python3 -m domaza.scrape dz-me-houses --detail-limit 20
...
from . import adapter as _adapter  # noqa: F401  — register domaza
...
def crawl_search(con, fetcher: Fetcher, name: str, spec: dict, max_pages: int | None,
    ...
    if want_details:
        ...
        # Country lists default to newest-first, not price. A capped
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape Domaza (domaza.com) into SQLite.")
    ap.add_argument("search", nargs="*", help="search names from searches.json (default: enabled domaza)")
    ...
    if foreign:
        ap.error(f"not a domaza search: {', '.join(foreign)}. "
