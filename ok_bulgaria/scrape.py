"""Scrape OK Bulgaria (cheap-bulgarian-house.co.uk) saved searches.

    python3 -m ok_bulgaria.scrape                     # enabled ok_bulgaria searches
    python3 -m ok_bulgaria.scrape bg-houses-50k
    python3 -m ok_bulgaria.scrape bg-houses-50k --no-details --max-pages 1
    python3 -m ok_bulgaria.scrape bg-houses-50k --detail-limit 20
...
from . import adapter as _adapter  # noqa: F401  — register ok_bulgaria
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape OK Bulgaria (cheap-bulgarian-house.co.uk) into SQLite.")
    ap.add_argument("search", nargs="*", help="search names from searches.json (default: enabled ok_bulgaria)")
    ...
    if foreign:
        ap.error(f"not an ok_bulgaria search: {', '.join(foreign)}. "
