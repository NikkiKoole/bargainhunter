"""Scrape Abruzzo Property Italy (abruzzopropertyitaly.com) saved searches.

    python3 -m abruzzopropertyitaly.scrape                          # enabled searches
    python3 -m abruzzopropertyitaly.scrape api-houses-100k
    python3 -m abruzzopropertyitaly.scrape api-houses-100k --no-details --max-pages 1
    python3 -m abruzzopropertyitaly.scrape api-houses-100k --detail-limit 20
...
from . import adapter as _adapter  # noqa: F401  — register abruzzopropertyitaly
...
def crawl_search(con, fetcher: Fetcher, name: str, spec: dict, max_pages: int | None,
    ...
    if want_details:
        ...
        # List pages are cheapest-first when order=priceasc; a capped
        # backfill should still hit the cheapest cards first.
        if todo:
            marks = ",".join("?" * len(todo))
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape Abruzzo Property Italy (abruzzopropertyitaly.com) into SQLite.")
    ap.add_argument("search", nargs="*",
                    help="search names from searches.json (default: enabled abruzzopropertyitaly)")
    ...
    if foreign:
        ap.error(f"not an abruzzopropertyitaly search: {', '.join(foreign)}. "
