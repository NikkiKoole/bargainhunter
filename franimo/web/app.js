...
let ALL = [], META = {}, VIEW = 'table', map = null, layer = null;
...
async function openDrawer(r) {
  if (!r) return;
  if (r.photos === undefined) {                 // heavy fields are fetched per listing
    r.photos = null;
    try {
      const full = await fetch('/api/listing/' + r.id).then(x => x.json());
