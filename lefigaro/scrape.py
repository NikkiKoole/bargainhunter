"""Scrape Le Figaro Immobilier (immobilier.lefigaro.fr) saved searches.

    python3 -m lefigaro.scrape                          # enabled lefigaro searches
    python3 -m lefigaro.scrape lf-23-houses-150k
    python3 -m lefigaro.scrape lf-23-houses-150k --no-details --max-pages 1
    python3 -m lefigaro.scrape lf-23-houses-150k --detail-limit 20

`python3 -m franimo.scrape` will refuse these — it only runs source=franimo.

A datacenter GET is Cloudflare-blocked. Run from a home IP (same class of
wall as Holprop) or from `cache/` after a home-IP fetch. Point trials at
`--db db/scratch.db`.
"""
...
from . import adapter as _adapter  # noqa: F401  — register lefigaro
from .http import BASE, DEFAULT_GAP, Fetcher
from .parse import SOURCE, is_blocked, parse_detail, parse_list
...
def crawl_search(con, fetcher: Fetcher, name: str, spec: dict, max_pages: int | None,
    ...
    url = spec["path"] if spec["path"].startswith("http") else BASE + spec["path"]
    while url:
        try:
            ...
            if "403" in str(e) or "Forbidden" in str(e):
                print(
                    "  ! Cloudflare blocked this IP. Le Figaro Immobilier needs a "
                    "home IP (same class of wall as Holprop).",
        ...
        if is_blocked(html):
            print(
                "  ! Cloudflare blocked this IP. Le Figaro Immobilier needs a "
                "home IP (same class of wall as Holprop). Cached pages in "
                "cache/ re-parse with zero requests.",
                file=sys.stderr, flush=True,
            )
            break
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape Le Figaro Immobilier (immobilier.lefigaro.fr) into SQLite.")
    ap.add_argument("search", nargs="*",
                    help="search names from searches.json (default: enabled lefigaro)")
    ...
    if foreign:
        ap.error(f"not a lefigaro search: {', '.join(foreign)}. "
