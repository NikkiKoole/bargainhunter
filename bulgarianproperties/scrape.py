"""Scrape Bulgarian Properties (bulgarianproperties.com) saved searches.

    python3 -m bulgarianproperties.scrape                     # enabled searches
    python3 -m bulgarianproperties.scrape bp-bg-under-10k
    python3 -m bulgarianproperties.scrape bp-bg-under-10k --no-details --max-pages 1
    python3 -m bulgarianproperties.scrape bp-bg-under-10k --detail-limit 20

`python3 -m franimo.scrape` will refuse these — it only runs source=franimo.

Browse/category URLs only. robots.txt Disallow's `/Search/`, `*minprice=`,
`*maxprice=`, `*page=` — never put those in searches.json.
"""
...
from . import adapter as _adapter  # noqa: F401  — register bulgarianproperties
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape Bulgarian Properties (bulgarianproperties.com) into SQLite.")
    ap.add_argument("search", nargs="*",
                    help="search names from searches.json (default: enabled bulgarianproperties)")
    ...
    if foreign:
        ap.error(f"not a bulgarianproperties search: {', '.join(foreign)}. "
