"""Scrape Holprop (holprop.com) saved searches.

    python3 -m holprop.scrape                          # enabled holprop searches
    python3 -m holprop.scrape hp-es-houses-100k
    python3 -m holprop.scrape hp-bg-houses-100k --no-details --max-pages 1
    python3 -m holprop.scrape hp-es-houses-100k --detail-limit 20
...
from . import adapter as _adapter  # noqa: F401  — register holprop
...
def crawl_search(con, fetcher: Fetcher, name: str, spec: dict, max_pages: int | None,
    ...
    if want_details:
        ...
        # Holprop list pages default to newest-first, not price. A capped
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape Holprop (holprop.com) into SQLite.")
    ap.add_argument("search", nargs="*", help="search names from searches.json (default: enabled holprop)")
    ...
    if foreign:
        ap.error(f"not a holprop search: {', '.join(foreign)}. "
