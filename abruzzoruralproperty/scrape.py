"""Scrape Abruzzo Rural Property (abruzzoruralproperty.com) saved searches.

    python3 -m abruzzoruralproperty.scrape                          # enabled searches
    python3 -m abruzzoruralproperty.scrape arp-houses-100k
    python3 -m abruzzoruralproperty.scrape arp-houses-100k --no-details --max-pages 1
    python3 -m abruzzoruralproperty.scrape arp-houses-100k --detail-limit 20
...
from . import adapter as _adapter  # noqa: F401  — register abruzzoruralproperty
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Scrape Abruzzo Rural Property (abruzzoruralproperty.com) into SQLite.")
    ap.add_argument("search", nargs="*",
                    help="search names from searches.json (default: enabled abruzzoruralproperty)")
    ...
    if foreign:
        ap.error(f"not an abruzzoruralproperty search: {', '.join(foreign)}. "
